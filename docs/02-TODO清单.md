...
- [~] A2A discovery、capability、delegation、result exchange：已定义 A2A peer/capability/delegation/output contract public 类型、register/list/delegate API、TUI `/a2a` 真实注册状态展示、`a2a.*` activity 过滤、`a2a.peer.discovered` 事件、最小 approval/audit delegation handler 和 result contract 校验；协议 transport 和完整 result exchange 仍待实现。
