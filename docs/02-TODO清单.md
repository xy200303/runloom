...
- [~] 事件回放和滚动：已支持 `/replay` 从 stored events 重建面板，`/session switch` 自动按目标 session 回放，并支持 `/clear` 清空当前视图、`/key` 分发 `Ctrl+L`、按当前 focus 滚动的 `PgUp/PgDn`、`Home/End`、`Alt+1/2/3` 和 Approval Center 快捷键、`/focus`、`/scroll` 命令式面板滚动；仍需真实 raw-key 捕获。
