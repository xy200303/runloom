...
import {
  createUnifiedDiff,
  summarizePatch
} from "./text-diff-tools.js";

const execFileAsync = promisify(execFile);
const SKIPPED_DIRS = new Set([".git", "node_modules", "dist", ".tsbuildinfo"]);

export function createListFilesTool(): ToolDefinition<{ path?: string; recursive?: boolean; maxEntries?: number }, { files: string[] }> {
  return {
    name: "fs.list",
...
export function createReadFileTool(): ToolDefinition<{ path: string; maxBytes?: number }, { path: string; content: string; truncated: boolean }> {
  return {
    name: "fs.read",
...
export function createPatchFileTool(): ToolDefinition<
