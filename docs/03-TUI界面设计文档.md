...
| `mcp.*` | MCP 状态区 |
| `external_agent.*` | external agent activity 与状态区 |
| `a2a.*` | 外部协作状态区 |
...
- `/todo`：显示 todo。
- `/activity [filter]`：显示 activity，可按 `runs`、`models`、`todos`、`coding`、`tools`、`approvals`、`reviews`、`skills`、`mcp`、`external` 过滤。
- `/activity view [filter] [index|latest|current]`：查看 activity 详情，包括事件类型、run/session、source、timestamp 和 payload；`current`、`selected` 或 `.` 指向当前滚动窗口顶部可见项。
- `/tools`：显示可用工具。
...
- `/key <shortcut>`：分发 line-mode 快捷键入口，支持 `ctrl+l`、`esc`、`enter`、`tab`、`shift+tab`、`pgup`、`pgdn`、`home`、`end`、`alt+1`、`alt+2`、`alt+3`、`alt+4`、`n`、`p`、`v`、`a`、`s`、`d`。
- `/quit`：退出。
...
- `Enter`：提交输入；line-mode `/key enter` 触发当前聚焦面板默认动作。
- `Shift+Enter`：换行。
- `Ctrl+C`：中止当前 run 或二次退出。
- `Esc`：关闭弹窗；line-mode `/key esc` 清除 approval focus 并回到 transcript。
- `Ctrl+L`：清屏。
- `Tab/Shift+Tab`：正向/反向切换聚焦面板。
- `PgUp/PgDn`：滚动当前聚焦面板。
- `Home/End`：跳到当前聚焦面板顶部/底部。
- `Alt+1`：聚焦 transcript。
- `Alt+2`：聚焦 todo。
- `Alt+3`：聚焦 activity。
- `Alt+4`：聚焦 status。
...
首期 readline 模式下，`Ctrl+L`、`Esc`、`Enter`、`Tab/Shift+Tab`、`PgUp/PgDn`、`Home/End`、`Alt+1/2/3/4` 和 Approval Center 的 `N/P/V/A/S/D` 通过 `/key <shortcut>` 复用同一套快捷键分发逻辑；`/key esc` 回到 transcript，`/key enter` 在 activity 面板展开当前可见 activity 详情，`Tab/Shift+Tab` 使用固定面板顺序轮转 focus，`PgUp/PgDn` 与 `Home/End` 使用当前 focus 的面板滚动状态，后续全屏 raw-mode 接入真实按键时不改变面板状态语义。
...
首期命令式界面通过 `/activity tools` 查看工具活动过滤视图，通过 `/activity tools view latest` 展开最近一条工具活动详情。
...
首期命令式界面支持 `/activity runs|models|todos|coding|tools|approvals|reviews|skills|mcp|external` 和 `/activity view ...`。详情视图支持按编号、`latest` 或当前滚动窗口顶部可见项展开。`/git`、`/diff`、`/tests` 这类由用户在 TUI 主动触发的 public tool API 结果也会记录到 `coding` activity，所有过滤与详情视图仍只消费 TUI view state。
...
首期命令式界面通过 `/a2a` 展示已注册的 A2A peers，包括 status、transport、endpoint、version 和 capability names；通过 `/external` 展示已注册的 external agent adapters，包括 kind、status、capabilities、command 和 maxTurns；`external_agent.*` 事件进入 `/activity external` 过滤视图；未注册时显示空状态，不注入产品 mock 数据。委托执行后续仍由 `runloom-agent` 的 approval/audit 路径承接。
