...
test("approval command updates scope and default modes", async () => {
  ...
  await app.runCommand("/scroll top");
  await app.runCommand("/key end");
  await app.runCommand("/key home");
  ...
  await app.runCommand("/key enter");
  await app.runCommand("/key tab");
  await app.runCommand("/key shift+tab");
  await app.runCommand("/key alt+4");
  await app.runCommand("/key esc");
  await app.runCommand("/todo");
  await app.runCommand("/diff");
  await app.runCommand("/tests pnpm typecheck");
  await app.runCommand("/activity coding bottom");
  await app.runCommand("/activity view coding latest");
  ...
  assert.match(text, /Activity Detail \(coding\): #35\n {2}category: coding\n {2}filteredIndex: 3\n {2}event: tool\.shell\.verify/);
